...
import { formatDate, convertToTrainCase } from './utils/commonUtils'; // Enforce single source of truth for date formatting
...
const operationHandlers: Record<string, Function> = {
  async addSiteUUID(frontmatter: Record<string, any>, filePath: string) {
    // Aggressive debugging log: entry
    ...
    if (!frontmatter.site_uuid) {
      ...
      // Aggressive debugging log: mutation
    ...
    // Aggressive debugging log: no change
  ...
  async updateDateModified(frontmatter: Record<string, any>, filePath: string) {
    // Aggressive debugging log: entry
    console.log(`[updateDateModified] file: ${filePath}`);
    console.log(`[updateDateModified] frontmatter before:`, JSON.stringify(frontmatter, null, 2));
    const { formatDate } = await import('./utils/commonUtils');
    frontmatter.date_modified = formatDate(new Date());
    // Aggressive debugging log: mutation
    console.log(`[updateDateModified] date_modified set:`, frontmatter.date_modified);
    console.log(`[updateDateModified] frontmatter after:`, JSON.stringify(frontmatter, null, 2));
    return { frontmatter, changed: true };
  },
  async fetchOpenGraph(frontmatter: Record<string, any>, filePath: string) {
    // Aggressive debugging log: entry
    console.log(`[fetchOpenGraph] file: ${filePath}`);
    console.log(`[fetchOpenGraph] frontmatter before OpenGraph:`, JSON.stringify(frontmatter, null, 2));
    ...
    if (ogResult && ogResult.changed && ogResult.updatedFrontmatter) {
      ...
      // Aggressive debugging log: mutation
      console.log(`[fetchOpenGraph] OpenGraph updated frontmatter:`, JSON.stringify(frontmatter, null, 2));
      return { frontmatter, changed: true };
    }
    // Aggressive debugging log: no change
    console.log(`[fetchOpenGraph] No OpenGraph changes for: ${filePath}`);
    return { frontmatter, changed: false };
  }
};
...
export class FileSystemObserver {
  ...
  private async processFileWithConfig(filePath: string, dirConfig: any): Promise<void> {
    ...
    // Type for operation steps in the handler sequence
    // This fixes TS7006: Parameter 'step' implicitly has an 'any' type.
    type OperationStep = { op: string; delayMs?: number };

    // Determine operation sequence (default to legacy behavior if not set)
    // Ensure updateDateModified is ONLY at the end
    let sequence: OperationStep[] = (dirConfig.operationSequence || [
      { op: 'addSiteUUID', delayMs: 25 },
      { op: 'fetchOpenGraph', delayMs: 25 },
      { op: 'validateFrontmatter', delayMs: 25 }
    ]) as OperationStep[];
    // Remove any existing updateDateModified steps
    sequence = sequence.filter((step: OperationStep) => step.op !== 'updateDateModified');
    // Add updateDateModified as the last step
    sequence.push({ op: 'updateDateModified', delayMs: 300 });

    // Extract frontmatter ONCE as a precursor before pipeline
    // Use static import for single source of truth
    let frontmatter = extractFrontmatter(await fs.readFile(filePath, 'utf8')) || {};
    let anyChange = false;
    for (let i = 0; i < sequence.length; i++) {
      const step: OperationStep = sequence[i];
      if (!operationHandlers[step.op]) {
        console.warn(`Unknown operation ${step.op} in sequence for ${filePath}`);
        continue;
      }
      // Only run updateDateModified at the end and only if there was a change
      if (step.op === 'updateDateModified' && !anyChange) {
        console.log(`No changes detected in previous steps for ${filePath}, skipping updateDateModified.`);
        continue;
      }
      const result = await operationHandlers[step.op](frontmatter, filePath);
      if (result && result.frontmatter) {
        frontmatter = result.frontmatter;
      }
      if (result.changed && step.op !== 'updateDateModified') {
        anyChange = true;
      }
      // Delay if specified
      if (step.delayMs) {
    ...
    // After ALL handlers and date_modified update, add to recentlyModifiedByObserver and start cooldown
  ...
  setupEventHandlers(): void {
    ...
    // On ready
    // REMOVE confusing message about initial scan completion
    // Previously: console.log('Initial scan complete. Ready for changes.');
